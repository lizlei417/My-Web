(() => {
  ...
  const injectStyles = () => {
    ...
    style.textContent = `
      ...
      .global-theme-toggle {
        ...
        border-radius: 24px;
        background: rgba(255,255,255,.72);
        box-shadow: 0 18px 46px rgba(var(--theme-rgb, 127,169,235), .14), inset 0 0 22px rgba(255,255,255,.7);
        backdrop-filter: blur(18px);
        cursor: pointer;
      }
      .global-theme-current {
        ...
        height: 42px;
        border-radius: 50%;
        background: var(--theme, #7fa9eb);
        box-shadow: 0 0 0 4px rgba(var(--theme-rgb, 127,169,235), .14), inset 0 0 0 2px rgba(255,255,255,.58);
      }
  ...
  const ensureGlobalThemeToggle = () => {
    ...
    const tools = document.querySelector(".global-top-tools");
    if (!tools || tools.querySelector(".global-theme-wrap")) return;
    const themes = [
  ...
  const ensureGlobalAccountDock = () => {
    ...
    const detail = isGuest
      ...
      : (savedEmail || "登录后，计划日记会同步到你的 Supabase 账号。");
