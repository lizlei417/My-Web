    ...
    const BUBBLE_INTRO_KEY = "portalBubbleIntroPending";
    ...
    const syncSettingsAccess = () => {
      const canUseSettings = Boolean(currentUser);
      settingsButton.setAttribute("aria-disabled", String(!canUseSettings));
      if (!canUseSettings) setSettingsMenuOpen(false);
    };
    ...
    const applySavedProfile = () => {
      ...
      const savedSignature = readValue("signature");
        const savedTheme = localStorage.getItem("theme")
          || (currentUser ? localStorage.getItem(userStorageKey("theme")) : readValue("theme"));
    ...
    const shouldPlayBubbleIntro = (nextUser) => {
      const signedInTransition = sessionResolved && !currentUser && Boolean(nextUser);
      const fromHiddenHome = Boolean(nextUser)
        && (document.body.classList.contains("logged-out") || document.body.classList.contains("auth-pending"));
      let requested = false;
      let firstSignedInLoad = false;
      try {
        requested = localStorage.getItem(BUBBLE_INTRO_KEY) === "1";
        if (requested) localStorage.removeItem(BUBBLE_INTRO_KEY);
      } catch {}
      try {
        if (!sessionResolved && nextUser) {
          const key = `portalBubbleIntroPlayed:${nextUser.id}`;
          firstSignedInLoad = sessionStorage.getItem(key) !== "1";
          if (firstSignedInLoad) sessionStorage.setItem(key, "1");
        }
      } catch {}
      return signedInTransition || requested || firstSignedInLoad || fromHiddenHome;
    };

    const applySession = (session) => {
      const nextUser = session?.user || (hasGuestSession() ? guestUser : null);
      const needsBubbleIntro = shouldPlayBubbleIntro(nextUser);
      ...
      if (currentUser?.isGuest) {
        try {
          localStorage.setItem("portalCurrentUserId", "guest");
        ...
        scheduleBubbleIntro(needsBubbleIntro);
      ...
      if (!currentUser) {
        try {
          localStorage.removeItem("portalCurrentUserId");
          localStorage.removeItem("portalCurrentUserEmail");
        } catch {}
        document.body.classList.add("logged-out");
        applyLoggedOutProfile();
        setAccountMessage("未登录", "登录后，网站中的数据内容可同步到你的 Supabase 账号。");
        accountSignUp.hidden = false;
        accountSignIn.hidden = false;
        accountEmail.hidden = false;
        accountPassword.hidden = false;
      ...
      document.body.classList.remove("logged-out");
      try {
        localStorage.setItem("portalCurrentUserId", currentUser.id || "");
    ...
    const defaultBubbleState = serializeBubbles();
    ...
    const placeBubble = (bubble) => {
      ...
      let x = Math.cos(angle) * rx + Math.sin(spin * .018 + phase) * drift;
      let y = Math.sin(angle) * ry + Math.cos(spin * .015 + phase) * drift * .72;

      if (bubble.classList.contains("bubble-entering")) {
        const startTime = Number(bubble.dataset.introStartTime || 0);
        const duration = Number(bubble.dataset.introDuration || 2600);
        ...
        if (progress >= 1) {
          ...
          bubble.style.filter = "";
          delete bubble.dataset.introStartTime;
          delete bubble.dataset.introDuration;
        } else {
          const eased = progress < .5
            ? 2 * progress * progress
            : 1 - Math.pow(-2 * progress + 2, 2) / 2;
          const targetAngle = Number(bubble.dataset.angle || 0) * Math.PI / 180;
          const avatarRect = avatarButton.getBoundingClientRect();
          const orbitRect = orbit.getBoundingClientRect();
          const center = bubbleCenter();
          const avatarRadius = Math.max(70, avatarRect.width / 2);
          const startSpread = Math.sin(targetAngle) * avatarRadius * .34;
          const startX = avatarRect.left + avatarRect.width / 2 - orbitRect.left - center.x + startSpread;
          const startY = avatarRect.top + avatarRect.height / 2 - orbitRect.top - center.y + avatarRadius * .58;
          const side = Math.sign(Math.cos(targetAngle)) || 1;
          const lift = Math.min(220, Math.max(110, Math.hypot(x - startX, y - startY) * .32));
          const controlX = startX + (x - startX) * .42 + side * lift * .48;
          const controlY = startY + (y - startY) * .42 - lift;
          const inv = 1 - eased;
          x = inv * inv * startX + 2 * inv * eased * controlX + eased * eased * x;
          y = inv * inv * startY + 2 * inv * eased * controlY + eased * eased * y;
          const scale = .18 + eased * .82 + Math.sin(eased * Math.PI) * .1;
          bubble.style.opacity = String(Math.min(1, progress * 1.45));
          bubble.style.filter = progress < .2 ? `blur(${(1 - progress / .2) * 7}px)` : "";
    ...
    const scheduleBubbleIntro = (enabled) => {
      if (!enabled) return;
      const bubbles = [...orbit.querySelectorAll(".bubble")];
      ...
      bubbles.forEach((bubble) => {
        seedBubbleMotion(bubble);
        bubble.dataset.introStartTime = String(introStartTime);
        bubble.dataset.introDuration = "2200";
    ...
    window.addEventListener("portal:sticky-login-required", () => {
      setAccountPanelOpen(true);
      showToast("登录后可使用便签");
    });
    ...
    window.addEventListener("message", (event) => {
      ...
      if (event.data?.type === "portal:apply-home-session") {
        const user = event.data.user;
        applySession(user?.id ? { user } : null);
        return;
      }
      if (event.data?.type === "portal:play-home-bubble-intro") {
        if (currentUser) {
          scheduleBubbleIntro(true);
        } else {
          try {
            localStorage.setItem(BUBBLE_INTRO_KEY, "1");
          } catch {}
        }
        return;
      }
      if (event.data?.type !== "portal:add-home-bubble") return;
      const shellUser = event.data.user;
      if (!currentUser && shellUser?.id) {
        currentUser = {
          id: shellUser.id,
          email: shellUser.email || "",
          isGuest: Boolean(shellUser.isGuest)
        };
        accountDock.classList.add("signed-in");
        syncSettingsAccess();
      }
      createBubble();
    });
    ...
    settingsMenu.addEventListener("click", (event) => {
      ...
      if (!action || action === "sign-out") return;
      if (!currentUser) {
        setSettingsMenuOpen(false);
        setAccountPanelOpen(true);
        showToast("登录后可调整组件");
      ...
      if (action === "reorder") {
        settingsMenu.querySelector("[data-global-action='save-order']").hidden = false;
        showToast("拖动组件后点击保存顺序");
        return;
      }
