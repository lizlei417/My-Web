...
const LOCAL_ENTRIES_KEY = "portalGuestPlanJournalEntries";
...
function writeLocalEntries(nextEntries) {
  try {
    localStorage.setItem(LOCAL_ENTRIES_KEY, JSON.stringify(nextEntries));
  } catch {}
}

function loadLocalEntries() {
  entries = readLocalEntries().map(rowToEntry);
  renderEntries();
}

function saveLocalEntry(payload) {
  const now = new Date().toISOString();
  const localRows = readLocalEntries();
  if (editingId) {
    const index = localRows.findIndex((entry) => entry.id === editingId);
    if (index >= 0) {
      localRows[index] = { ...localRows[index], ...payload, id: editingId, updated_at: now };
    }
  } else {
    localRows.push({
      ...payload,
...
function renderEntries() {
  ...
  filtered.forEach((entry) => {
    ...
    if (entry.tags.length) {
      sortTags(entry.tags).forEach((tag) => tagsWrap.append(createPlainTag(tag)));
    } else {
      tagsWrap.remove();
    }
...
async function toggleEntry(id) {
  ...
  renderEntries();
  if (currentUser?.isGuest) {
    const localRows = readLocalEntries().map((row) => (
      row.id === id ? { ...row, expanded: entry.expanded, updated_at: new Date().toISOString() } : row
    ));
    writeLocalEntries(localRows);
    return;
  }
...
function editEntry(id) {
  ...
  selectedTags = sortTags(entry.tags);
  renderSelectedTags();
...
async function deleteEntry(id) {
  ...
  if (currentUser?.isGuest) {
    writeLocalEntries(readLocalEntries().filter((item) => item.id !== id));
  } else {
...
function clearForm({ preserveColor = false } = {}) {
  editingId = null;
  form.reset();
  dateInput.value = todayISO;
  if (!preserveColor) {
...
function getHomeThemeColor(userId = "") {
  let savedTheme = null;
  try {
      const savedThemeText = localStorage.getItem("theme")
        || (userId ? localStorage.getItem(`user:${userId}:theme`) : null);
...
function addTagFromInput() {
  ...
  selectedTags = sortTags([...selectedTags, tag]);
  tagInput.value = "";
  renderSelectedTags();
}
...
function sortTags(tags) {
  return [...tags].sort((a, b) => a.localeCompare(b, "en", { numeric: true, sensitivity: "base" }));
}
...
function rowToEntry(row) {
  return {
    ...
    tags: Array.isArray(row.tags) ? sortTags(row.tags) : [],
