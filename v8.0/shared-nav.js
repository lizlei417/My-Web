(() => {
  ...
  const DEFAULT_ORDER = ["home", "habitat", "schedule", "bookkeeping", "journal", "bookmarks", "notes"];
  ...
  const pageRoot = (() => {
    ...
    if (path.includes("/schedule/")) return "..";
    if (path.includes("/bookkeeping/")) return "..";
    if (path.includes("/plan-journal/")) return "..";
    if (path.includes("/collect/")) return "..";
